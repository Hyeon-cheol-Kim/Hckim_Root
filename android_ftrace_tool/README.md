...
2. **ftrace 옵션 설정** — **스토리지 스택(UFS / SCSI / F2FS / block)** 관련 이벤트
   그룹만 한 줄 설명과 함께 보여줍니다(디바이스가 실제 지원하는 것만 선택 가능).
   번호를 입력하면 해당 그룹이 즉시 on/off 토글되며, `s`(설정완료)를 누르기
   전까지 자유롭게 선택할 수 있습니다. `x` 를 누르면 전체 그룹 보기(고급)로
   전환할 수 있습니다.
   ...
   > 관심 분야를 바꾸려면 `core.py` 의 `STORAGE_EVENT_GROUPS` 딕셔너리만 수정하면 됩니다.
